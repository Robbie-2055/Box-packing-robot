...
//function to gradually move a servo to its target angle
...
void recordServoAngles(){
  //Check if there is space to make more recordings
  if(recordIndex < maxRecordings){
    recordedAngles[0][recordIndex] = prevBaseAngle; //Save current base angle
    recordedAngles[1][recordIndex] = prevShoulderAngle;//save current shoulder angle
    recordedAngles[2][recordIndex] = prevElbowAngle;//save current elbow angle
    recordedAngles[3][recordIndex] = prevWristAngle;//save current wrist angle
...
void recordMode() {
  ...
  //Call the teleoperation to all user to manually control the arm while in record mode
  teleoperationMode();

  //check if there is space to make another recording
  if (recordIndex >= maxRecordings){
...
void moveToTarget(Servo &servo, int targetAngle, int &currentAngle) {
  ...
  while (currentAngle != targetAngle) {
    ...
    // check if switch mode button has been pressed
    if(digitalRead(buttonPin) == HIGH){
    ...
    //check for button press to erase all data within array
    if(digitalRead(SW_pinJoystick2) == LOW){
      Serial.println("Removed all recordings");
      //LED indication for removal of all saved recordings
      digitalWrite(whiteLedPin, LOW);
      delay(250);
      digitalWrite(whiteLedPin, HIGH);
      delay(250);
      
      recordIndex = 0;// deleting all the contents of t6he array
    }
